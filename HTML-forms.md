...
*(However, if it is inside the `<form>`, as `<button>Submit me!</button>` or `<button type="submit">Submit me!</button>`, it automaitcally submits the form. But, if it set to 'button', like: `<button type="button">No submit</button>`, it won't submit the form.)*
