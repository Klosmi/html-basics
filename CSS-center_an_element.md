...
In this case, if the content is more than the mentioned height, than it [overflows](https://developer.mozilla.org/en-US/docs/Web/CSS/overflow).    
	...
	With `min-height` property we can set a minimum height for the element to occupy the space if the content in the element is less than that and the height of the element will increase when the content increases.
